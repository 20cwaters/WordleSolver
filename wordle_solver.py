...
def load_past_words(filename="past_used_words.txt"):
    ...
    try:
        with open(filename, 'r') as file:
            content = file.read()
            # Past words are separated by " | "
            words = [word.strip().lower() for word in content.split('|')]
            PAST_WORDS = set(words)
        print(f"Loaded {len(PAST_WORDS)} past used Wordle words to exclude")
    except FileNotFoundError:
        print(f"Warning: {filename} not found. No past words will be excluded.")
        PAST_WORDS = set()
    return PAST_WORDS
...
def load_words(filename="words.txt"):
    ...
    try:
        with open(filename, 'r') as file:
            all_words = [word.strip().lower() for word in file if len(word.strip()) == 5 and word.strip().isalpha()]
            
        # Filter out past used words
        if PAST_WORDS:
            initial_count = len(all_words)
            WORD_LIST = [word for word in all_words if word not in PAST_WORDS]
            excluded_count = initial_count - len(WORD_LIST)
            print(f"Excluded {excluded_count} past used words")
        else:
            WORD_LIST = all_words
            
        print(f"Successfully loaded {len(WORD_LIST)} words from {filename}")
    except FileNotFoundError:
        print(f"Warning: {filename} not found. Using small sample list instead.")
        # Common five-letter words, good for starting
        sample_words = [
        ...
        WORD_LIST = [word.lower() for word in sample_words if len(word) == 5 and word.isalpha() and word not in PAST_WORDS]
    return WORD_LIST
